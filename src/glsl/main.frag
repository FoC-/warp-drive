...
void mainImage(out vec4 O, vec2 C) {
  float i     // Loop counter (starts at 0)
  , d     // Distance to nearest surface
  , z = fract(dot(C, sin(C))) - .5f  // Ray distance + noise for anti-banding
  ;
  vec4 o     // Accumulated color/lighting
  , p     // Current 3D position along ray
  ;
  for(vec2 r = iResolution.xy  // Screen resolution
    ; ++i < 77.f; z += .6f * d                // Step forward (larger steps when far from surfaces)
    )
      // Convert 2D pixel to 3D ray direction
    p = vec4(z * normalize(vec3(C - .5f * r, r.y)), .1f * iTime)
    ...
    , p.xy *= mat2(cos(2.f + O.z + vec4(0, 11, 33, 0)))
    ...
    , O = (1.f + sin(.5f * O.z + length(p - O) + vec4(0, 4, 3, 6))) / (.5f + 2.f * dot(O.xy, O.xy))
    ...
    , d = abs(min(length(p.xy) - .125f, min(p.x, p.y) + 1e-3f)) + 1e-3f
